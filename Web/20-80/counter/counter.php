...
if(!mysql_num_rows(mysql_query("SELECT ip_address FROM info WHERE ip_address = '$ip' AND app='$app'"))) // check if the IP is in database
{
	// if not , add it.	
	$adddata = mysql_query("INSERT INTO $dbtableinfo (ip_address, user_agent, datetime, app) VALUES('$ip' , '$agent','$datetime', '$app' ) ") ;
	if (!$adddata) 
	{
